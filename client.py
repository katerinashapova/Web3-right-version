...
from data.config import TOKEN_ABI, privat_key, arb_rpc 
...
class Client:
    ...
    def __init__ (
      ...
      self.address = Web3.to_checksum_address(self.w3.eth.account.from_key(privat_key).address)
    ...
    def check_balance_interface(self, token_address, min_value) -> bool:
       ...
       if balance < min_value * 10 ** decimal:
          print(f'{self.address}|balanceOf|not enougt{token_address}')
          return False
       return True
    ...
    def send_transaction(
      ...
      sign = self.w3.eth.account.sign_transaction(tx_params, self.privat_key)
